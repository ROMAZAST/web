...
bot.onText(/^\/menu/, (msg) => {
  const photoUrl = 'https://media.discordapp.net/attachments/791782853209358379/1112689263537049600/Frame12_1.png';
...
bot.on('callback_query', (callbackQuery) => {
  ...
  if (callbackQuery.data === 'help') {
    const message = callbackQuery.message;
    const targetBotLink = 'linktobot';
    const targetBotLinkForMesg = 'linktobot';
    ...
    getUser(chatId).then(user => {
      ...
      {
        open_shared_lists(callbackQuery)
      }
      else if(user.auth == null)
      {
        setAuthUser(chatId, false)
    ...
    getUser(chatId).then(user => {
      ...
      {
        openList(id_list).then(list => {
        openListbykeys(callbackQuery,list)
    ...
    getUser(chatId).then(user => {
      ...
      {
        delete_list(id_list).then(result =>{
          console.log(result)
          lists_out(callbackQuery, `Список видалено`, 1)
        })
      }
      else if(user.auth == null)
      {
    ...
    getUser(chatId).then(user => {
      ...
      {
        add_product(callbackQuery, _id, page)
      }
      else if(user.auth == null)
      {
        setAuthUser(chatId, false)
    ...
    getUser(chatId).then(user => {
      ...
      { 
        open_section(callbackQuery, name_sect, _id, id_sect,page)
      }
      else if(user.auth == null)
      {
        setAuthUser(chatId, false)
    ...
    getUser(chatId).then(user => {
      ...
      { 
        share_list_confirm(callbackQuery, _id)
      }
      else if(user.auth == null)
      {
        setAuthUser(chatId, false)
    ...
    getUser(chatId).then(user => {
      ...
      { 
        if(count !== 0){
          confirm_menu(callbackQuery,name,_id, count,id_sect)}
          else if(count == 0)
          {
            bot.answerCallbackQuery(callbackQuery.id,{ text: `Додати до списку можна не менше, ніж 1 продукт`});
          }
      }
      else if(user.auth == null)
      {
    ...
    getUser(chatId).then(user => {
      ...
      { 
        add_element_to_list(callbackQuery,name_prod,count,_id)
      }
      else if(user.auth == null)
      {
        setAuthUser(chatId, false)
    ...
    const id_sect = data1.split(':')[1];//  id секції
    const name = data1.split(':')[2]; // id productu
    const _id = data1.split(':')[3];
    const count = data1.split(':')[4];
    console.log(name +` adwad ` +_id)
    getUser(chatId).then(user => {
    ...
    const _id = data1.split(':')[1];//  id секції
    ...
    getUser(chatId).then(user => {
      ...
      { 
        if (admins_id.indexOf(String(id)) !== -1){
        openList(_id).then(list => {
          openShareListbykeys_for_admins(callbackQuery,list)
        ...
        openList(_id).then(list => {
          if(list !== null)
          {
          openShareListbykeys_for_user(callbackQuery,list)
          }
    
        });
      }
      console.log(` adwad ` +_id)
      }
      else if(user.auth == null)
      {
        setAuthUser(chatId, false)
